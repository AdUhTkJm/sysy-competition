...
TODO：分析整数范围。也许可以弄一个不需要 e-SSA 的形式？
...
利用 Lisp-like DSL 进行匹配与改写。这个 DSL 在编译器的其他地方（例如识别循环变量时，以及 ARM 后端中）也有运用。
...
基于这个 Pass 衍生出了 EarlyConstFold 以及 LateConstFold 两个 Pass，会做更多不便用这种 DSL 描述的折叠（例如与数组有关的折叠）。
...
TODO：目前有 bug，只能展开 2 次。而且展开之后效率贼低。

对执行常数次的循环有特别处理：这时，如果可以直接展开整个循环，开头的 phi 就不需要存在了。直接取 preheader 的值即可。
